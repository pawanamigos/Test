...
def get_images():
    ...
    session = get_session()
    memo = simple_page_view(payload, session, Image, "created_timestamp")
    session.close()
    return memo
...
def get_image(image_id: str):
    session = get_session()
    memo = model_by_id(image_id, session, Image).to_payload()
    session.close()
    return memo
